...
void setup() {
  ...
  digitalWrite(aux_control,LOW); //make sure all output pins are low after boot 
...
void tone8() {
 ...
 void set_timeout() {
  ...
  if (readString.length() == 5) {
    ...
    if (readString == "#5933") {//Club ID4
       mute_off();
       tx_on();
       delay (1000);
        mp3.playFileByIndexNumber(92);
      check_busy();
      tx_off();
    }
    if (readString == "#5934") {//ID5
       mute_off();
       tx_on();
       delay (1000);
        mp3.playFileByIndexNumber(96);
    ...
    if (readString == "#8520") {//auxiliary off
       mute_off();
       tx_on();
    ...
    if (readString == "#8521") {//auxiliary on
       mute_off();
       tx_on();
       digitalWrite(aux_control,HIGH);
       delay (1000);
        mp3.playFileByIndexNumber(79);//if you want to add voice confirmation insert command below.
       delay(2000);
       tx_off();
    }
    if (readString == "#8522") {//step_up pulse on
    ...
    if (readString == "#8523") {//step_down pulse on
    ...
    if (readString == "#7934") {
      ...
      delay(640);
       mp3.playFileByIndexNumber(79); // say access granted
      delay(2000);
       mp3.playFileByIndexNumber(106); // say enter new year
      delay(5000);
      tx_off();
    }
